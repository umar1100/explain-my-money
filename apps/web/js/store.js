 ...
 *        categorySource ('auto'|'rule'|'user'|null), categoryConfidence,
 *        categoryReason, spendAmountMinor, excluded, confidence, status,
 *        receiptId, _error)
 ...
 *   householdRules(id, enabled, priority, matchMerchant, kind,
 *                  category (flat category id; legacy category rules keep
 *                  appMatch.setCategory instead), appMatch {merchantContains,
 *                  setCategory} (legacy category-rule payload), label, source,
 *                  createdAt)
 *     Rule identity is generated by Engine.makeRuleFromCorrection; kind-only
 *     rule id/hash shape is unchanged. No IndexedDB migration is needed for
 *     the category fields: txns and householdRules records are schemaless
 *     (no new index required) — v3 stands.
