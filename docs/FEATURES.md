...
| Merchant lookup (online, off by default): identifies merchants the built-in rules don't recognize with one web search per merchant (Tavily). Only the merchant name is ever sent — never amounts, dates, accounts, IDs (merchant-only payload asserted before every send). Clear identifications (≥2 agreeing results) auto-categorize and become household rules; weak evidence shows as a "web suggests: …" hint in Review; ambiguous stays silent. Each merchant looked up once ever (on-device cache). Key stored on this device only (never committed/proxied); pasted keys are whitespace-trimmed before saving; deleting it disables the feature. More → Merchant lookup includes a "Test connection" button: one probe request with the synthetic query "TEST MERCHANT" (never a real merchant) that reports key rejected / quota used up / offline / blocked (VPN, iCloud Private Relay, content blocker) in plain language. Triggered after import completes and after More → Auto-categorize; failures (offline, bad key, quota) degrade silently to Review | `apps/web/js/lookup.js` (Lookup: settings, cache, candidates, buildRequest, assertMerchantOnly, run/processTxns, makeRule, statusText, TEST_QUERY, networkHint, testConnection); `app.js` `App.lookupOn`, `App.vLookup`, `App.lookupSettingsHtml`, `App.Changes['lookup-enabled'/'lookup-key']`, `App.Actions['lookup-forget-key'/'lookup-test']`, `App.runMerchantLookup`, hooks in `App.runPipeline` + `App.runAutocatBackfill`, hint chip in `App.txnRowHtml` + `App.vTxnDetail`, `.lookup-toast` in `apps/web/css/styles.css`; sw.js `emm-shell-v13` | merchant-lookup.node.js (51+ checks); browser smoke **not yet** automated |
